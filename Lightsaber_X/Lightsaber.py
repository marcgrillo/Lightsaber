...
class Beam:
    def __init__(self, config_data, seed=None, plot_dir=False):
        ...
        self.type = config_data['type']                                     # type of component (Sensor)
        self.BS_offset = np.array(config_data['BS_offset'])                 # intentional beam-spot offset [m]
        ...
        self.torque = 0.                # instantaneous radiation-pressure torque [Nm]
        self.ti = 0                     # index running through input noise batch
    ...
    def step(self, pitch, input_power):
        ...
        self.torque = 2 / 299792458.0 * self.P * (self.BS+self.BS_offset) - torque_dc
...
class Mirror:
    def __init__(self, config_data, seed=None, plot_dir=False):
        ...
        self.angle = 0.  # mirror pitch angle [rad]
        self.ti = 0  # index running through input noise batch
        self.R = 1 - self.T
    ...
    def step(self, rad_torque=0, actuation=0):

        # angular motion from radiation-pressure torque
        angle_rad, zf = utils.faster_sosfilt(self.rad_to_angle_sos, np.array([rad_torque]), zi=self.rad_to_angle_sos_state)
        self.rad_to_angle_sos_state = zf

        # angular motion from angular controls
        angle_act, zf = utils.faster_sosfilt(self.act_to_angle_sos, np.array([actuation]), zi=self.act_to_angle_sos_state)
        self.act_to_angle_sos_state = zf

        # add inputs to angular motion (contains radiation-pressure compensation)
        self.angle = self.sus_noise_tt[self.ti] + angle_rad[0] + angle_act[0]
...
class Sensor:
    def __init__(self, config_data, seed=None, plot_dir=False):
        ...
        self.type = config_data['type']                             # type of component (Sensor)
        self.fs = config_data['simulation_sampling_frequency']      # sampling frequency [Hz]
        self.T_batch = config_data['duration_batch']                # duration of simulated batch [s]
        self.T_fft = config_data['duration_fft']                    # duration of FFT segment [s]
    ...
    def step(self, dof):
        self.obs = eval(self.matrix) @ dof + self.readout_noise_tt[:, self.ti]
        self.ti += 1
        return self.obs
...
class Controller:
    ...
    def step(self, control_in):
        ...
        return -(matrix_num @ self.actuation).flatten()
...
def run(system, simulation):
    ...
    N = eval(str(simulation['duration_batch']*simulation['simulation_sampling_frequency']))

    in_power_t0 = 0.
    rad_torque_t0 = np.array([0., 0.])
    pitch_t0 = np.array([0., 0.])
    readout_t0 = np.array([0., 0.])
    actuation_t0 = np.array([0., 0.])
    pitch_tt = np.zeros((N, 2))
    cavity_power_tt = np.zeros((N, ))
    actuation_tt = np.zeros((N, 2))
    beamspot_tt = np.zeros((N, 2))
    readout_tt = np.zeros((N, 2))

    laser = system.components[0]
    itm = system.components[1]
    beam = system.components[2]
    etm = system.components[3]
    sensor = system.components[4]
    controller = system.components[5]
    rp_comp = system.components[6]
    for k in tqdm.tqdm(range(N-1), disable=not sys.stdout.isatty()):
        in_power_t1 = laser.step()
        if simulation['loop'] == 'open':
            rad_torque_t1 = beam.step(pitch_t0, in_power_t0)
        elif simulation['loop'] == 'closed':
            rad_torque_t1 = beam.step(pitch_t0, in_power_t0) - beam.eigen_to_local @ rp_comp.step(readout_t0)
        pitch_t1 = np.array([itm.step(rad_torque_t0[0], actuation_t0[0]), etm.step(rad_torque_t0[1], actuation_t0[1])])
        readout_t1 = sensor.step(pitch_t0)
        actuation_t1 = controller.step(readout_t0)

        cavity_power_tt[k] = beam.P
        beamspot_tt[k, :] = beam.BS
        pitch_tt[k, :] = pitch_t1
        readout_tt[k, :] = readout_t1
        actuation_tt[k, :] = actuation_t1

        in_power_t0 = in_power_t1
        rad_torque_t0 = rad_torque_t1
        pitch_t0 = pitch_t1
        readout_t0 = readout_t1
        if simulation['loop'] == 'closed':
            actuation_t0 = actuation_t1

        if np.all(np.abs(0.0) > 1):
    ...
    return [[pitch_tt, "rad", "pitch", "Pitch (ITM/ETM)"], [cavity_power_tt, "W", "power", "Cavity power"],
            [actuation_tt, "Nm", "actuation", "Control output (ITM/ETM)"], [beamspot_tt, "m", "beamspot", "Beam-spot motion (ITM/ETM)"],
