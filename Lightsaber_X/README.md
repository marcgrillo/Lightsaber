...
│   ├── main.tex                  ← the paper being written (canonical)
│   ├── ra_ts_*.tex               RA-TS algorithm / theory / skeleton write-ups
│   ├── li_tar_ucb_draft.tex      LI-TAR-UCB write-up
│   ├── cg_iclb_*.tex             CG-ICLB algorithm write-ups
│   ├── figs/                     paper figures
│   └── archive/                  superseded drafts (lightsaber_draft, tar_ucb_draft, …)
│
...
- `switching_dynamics/`, `paper_characterization/` — figure-output directories that
  `paper/main.tex` includes from (regenerate them locally before building the paper);
- `__pycache__/`, Numba caches, LaTeX build artifacts.

Regenerate any of these with the commands above.
