...
def hash_params(params: Dict[str, Any]) -> str:
    """Creates a consistent MD5 hash from a dictionary."""
    # Convert dict to JSON string with sorted keys for consistency
    # We use a custom encoder to handle types that might not be serializable
    # but for typical config (floats, strings, tuples), standard is fine.
    # Convert tuples to lists for JSON consistency if needed, but str() rep is also fine for hashing.
    s = json.dumps(params, sort_keys=True, default=str)
    return hashlib.md5(s.encode('utf-8')).hexdigest()

def get_cached_noise_bank(cache_dir: str, params_hash: str, generator_func, *args, **kwargs):
    """
    Tries to load noise bank from cache. If not found, generates and saves it.
    
    Args:
        cache_dir: Directory to store cache files.
        params_hash: Unique hash identifying the configuration.
        generator_func: Function to call if cache miss.
        *args, **kwargs: Arguments passed to generator_func.
        
    Returns:
        The noise bank object.
    """
    os.makedirs(cache_dir, exist_ok=True)
    cache_path = os.path.join(cache_dir, f"noise_bank_{params_hash}.pkl")
    
    if os.path.exists(cache_path):
        print(f"Loading cached noise bank from: {cache_path}")
        try:
    ...
    # Regenerate
    print(f"Cache miss (or load failed). Generating noise bank...")
    noise_bank = generator_func(*args, **kwargs)
    
    print(f"Saving noise bank to cache: {cache_path}")
    try:
        with open(cache_path, 'wb') as f:
            pickle.dump(noise_bank, f, protocol=pickle.HIGHEST_PROTOCOL)
    except Exception as e:
