...
\begin{figure}[t]
  ...
  raw reward score, one panel per regime, pooling all $745$ settled \SI{2}{s}
  blocks per (regime, controller) cell ($149$ blocks $\times$ $5$ independent
  noise realisations; continuous-filter convention). The distributions are
  broad and strongly overlapping---the per-block noise is far larger than the
  mean separations---so a single observation carries little information about
  the acting regime or the best controller; only window-averaging and
  accumulation across windows resolve the ordering. The regime signatures are
  visible: in $R_0$ the ordering $\Cz\!>\!\Co\!>\!\Ct$ shows as a rightward
  shift of $\Cz$ (blue); in $R_1$ all three nearly coincide; in $R_2$ the
  ordering reverses, with $\Cz$ shifted left. \emph{Bottom:} mean logistic
  (bandit-space) reward per regime and controller; bar heights and error bars
  are the mean and standard error \emph{of the five per-seed mean rewards}
  (one mean per $149$-block realisation), not of the $745$ pooled blocks
  shown above---the blocks within a realisation are autocorrelated (continuous
  filtering) and only the five realisations themselves are independent. The
  diagonal (regime-optimal) controller is
