...
\begin{figure}[t]
  ...
  noise-split duplicates, see text); rolling fraction-optimal for RA-TS-F, the
  rule-based policy, and D-UCB. RA-TS-F recovers to near-optimal play within
  each regime episode, while the baselines' recovery is slower and, for the
  rule-based policy, gated on its alarm and periodic re-check schedule.}
...
\begin{figure}[t]
  ...
  Oracle, RA-TS-F, and the rule-based policy. RA-TS-F's schedule visually
  mirrors the Oracle's ($\sim$\,$17$k switches vs.\ the Oracle's $865$, the
  excess being brief acquisition probes); the rule-based policy switches
  rarely ($1683$) and visibly lags the severity pattern, its re-checks being
  schedule- rather than event-driven.}
...
drift detector rather than scheduled. Fraction-optimal illustrates why regret,
