...
raw and logistic-mapped reward; Fig.~\ref{fig:reward} shows both the full
per-controller reward \emph{distributions} in each regime (top) and the mean
logistic (bandit-space) reward with error bars (bottom).
...
\begin{figure}[t]
  ...
  \caption{Reward landscape. \emph{Top:} per-controller distributions of the
  raw reward score (\SI{2}{s} blocks, continuous-filter convention), one panel
  per regime. The distributions are broad and strongly overlapping---the
  per-block noise is far larger than the mean separations---so a single
  observation carries little information about the acting regime or the best
  controller; only window-averaging and accumulation across windows resolve the
  ordering. The regime signatures are visible: in $R_0$ the ordering
  $\Cz\!>\!\Co\!>\!\Ct$ shows as a rightward shift of $\Cz$ (blue); in $R_1$ all
  three nearly coincide; in $R_2$ the ordering reverses, with $\Cz$ shifted
  left. \emph{Bottom:} mean logistic (bandit-space) reward per regime and
  controller; error bars are the standard error of the mean over the five
  independent noise realisations. The diagonal (regime-optimal) controller is
  largest in each regime and the reward is non-saturating (all $0.31$--$0.60$),
  but the separations are regime-dependent: large in $R_0$, small in $R_1$, and
  marginal in $R_2$ where $\Co$ and $\Ct$ are statistically indistinguishable
  (error bars overlap). What identifies a regime is the \emph{shape} of the
  mean-reward vector across controllers, not its overall level (similar in
  $R_0$ and $R_2$).}
  \label{fig:reward}
\end{figure}
