...
noise realisations (seeds) of \SI{360}{s} each. The band filters, run
continuously from $t=0$, take time to reach their steady-state response, so
each realisation's first \SI{60}{s} (its filter \emph{transient}) are discarded
and only the remaining, \emph{settled} blocks---those computed once the
filters' output no longer depends on their arbitrary start-up condition---are
kept: $149$ settled \SI{2}{s} blocks per realisation, $745$ pooled over the
five realisations for each (regime, controller) combination. This pooled set
of $745$ blocks is what the histograms of Fig.~\ref{fig:reward} (top) show.
Because the $149$ blocks within one realisation come from that same
continuously-running filter, consecutive blocks are autocorrelated and cannot
be treated as independent draws for an error estimate; we therefore first
average the $149$ blocks of each realisation down to a single per-realisation
mean reward, and report the bar heights and error bars of
Fig.~\ref{fig:reward} (bottom) as the mean and standard error \emph{of those
five per-realisation means} (five genuinely independent samples), not of the
$745$ raw blocks. Table~\ref{tab:reward} reports the corresponding mean raw
and logistic-mapped reward.
...
\begin{figure}[t]
  ...
  controller and regime. Each panel pools $745$ blocks per controller: $5$
  independent \SI{360}{s} noise realisations, each contributing $149$
  \emph{settled} blocks (i.e.\ computed after discarding the first \SI{60}{s},
  during which the continuously-running band filters are still in their
  start-up transient). \emph{Bottom:} each
